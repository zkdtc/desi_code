...
for expid in expid_arr:
    ...
    in_dir='/project/projectdirs/desi/spectro/redux/daily/exposures/'+night+'/'+expid
    ...
    for i in range(len(camera_arr)):
        ...
        #print(cmd)
        #os.system(cmd)
        cmd='desi_process_exposure -i '+in_dir+'/frame-'+camera_arr[i]+'-'+expid+'.fits --fiberflat /global/cfs/cdirs/desi/spectro/redux/daily/calibnight/20200313/fiberflatnight-'+camera_arr[i]+'-20200313.fits -o '+out_dir+'/flatfielded-by-individual-frame-'+camera_arr[i]+'-'+expid+'.fits'
        print(cmd)
        os.system(cmd)
