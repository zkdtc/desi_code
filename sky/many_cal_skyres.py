...
sp_num_arr=['4','8','9','0','2','3','6','7','5','1']
tail=''
date_arr=['20200306','20200306']
expnum_arr=['00053741','00053743']
product='andes'
"""
...
for i in range(n_exp):
    date=date_arr[i]
    expnum=expnum_arr[i]   
    in_dir='/project/projectdirs/desi/spectro/redux/'+product+'/exposures/'+date+'/'+expnum
    for cam in cam_arr:
        print(cam)
        for j in range(len(sp_num_arr)):
            ...
            #cmd='nohup python3 cal_skyres_skyonly.py -i /project/projectdirs/desi/spectro/redux/daily/exposures/'+date+'/'+expnum+'/frame-'+cam+sp_num+'-'+expnum+'.fits --fiberflat /project/projectdirs/desi/spectro/desi_spectro_calib/trunk/spec/sp'+sp_num+'/fiberflat-sm'+str(int(sp_num)+1)+'-'+cam+'-20191108.fits -o '+'frame-'+cam+sp_num+'-'+expnum+'.fits.dat -m rough >'+'frame-'+cam+sp_num+'-'+expnum+'.fits.log &'
            #print(cmd)
            #os.system(cmd)
